...
Khi nhập thông số trong admin, dùng mỗi dòng theo mẫu:

```text
Nhóm | Tên thông số | Giá trị
```

Ví dụ:

```text
Tổng quan | Loại Tivi | Smart Tivi QLED
Tổng quan | Kích cỡ màn hình | 55 inch
Công nghệ hình ảnh | Bộ xử lý | Q4 AI Processor
```

Dữ liệu này được lưu vào trường `specifications` và trang chi tiết sẽ nhóm các dòng theo “Nhóm” để hiển thị trong modal **Thông số chi tiết** trên cùng trang.
