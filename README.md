...
- `tivi-cu-da-nang.html`: trang landing local SEO cho nhu cầu tìm mua tivi cũ tại Đà Nẵng.
- `sua-tivi-da-nang.html`: trang landing local SEO cho nhu cầu kiểm tra, tư vấn sửa tivi tại Đà Nẵng.
...
- `styles.css`: thêm giao diện admin responsive, landing page responsive, giữ phong cách trắng + xanh đậm và font Aptos.
- `sitemap.xml`: bổ sung hai trang landing local SEO mới, không đưa `admin.html` vào sitemap.
...
## 3.1.1. Trang landing local SEO

Website có thêm hai trang landing local SEO nhưng vẫn thuộc cùng hệ thống Anh Minh Store:

- `tivi-cu-da-nang.html`: phục vụ khách tìm **tivi cũ Đà Nẵng**, mua tivi cũ, tivi cũ giá rẻ, tivi cũ có bảo hành và thu cũ đổi mới. Trang dùng dữ liệu sản phẩm công khai từ Supabase khi có sản phẩm loại `Tivi cũ`; nếu chưa có hàng phù hợp thì hiển thị lời nhắn gọi hotline tư vấn mua bán `0702386544` / `0389660779`.
- `sua-tivi-da-nang.html`: phục vụ khách tìm **sửa tivi Đà Nẵng**, các lỗi như sọc màn, mất nguồn, có tiếng không hình, treo logo, lỗi kết nối. Trang dùng hotline kỹ thuật `0905111223` / `0774111223` và nội dung tư vấn an toàn, không hứa giá hoặc kết quả sửa khi chưa kiểm tra.

Hai trang này được liên kết từ trang chủ, khu vực dịch vụ liên quan và footer để người dùng cũng như công cụ tìm kiếm dễ phát hiện. `sitemap.xml` đã được cập nhật để thêm hai URL mới, đồng thời vẫn không thêm `admin.html`. Mỗi trang có JSON-LD cho cửa hàng/dịch vụ/câu hỏi thường gặp phù hợp, có địa chỉ, số điện thoại, giờ mở cửa và khu vực phục vụ Đà Nẵng; không có dữ liệu đánh giá, sao xếp hạng hoặc review giả.
...
## 7.3. Ghi chú bảo mật admin một tài khoản dùng chung
`admin.html` vẫn là một file public vì GitHub Pages chỉ phục vụ website tĩnh; thẻ `noindex` chỉ giúp hạn chế công cụ tìm kiếm lập chỉ mục, không phải lớp bảo mật chính. Bảo vệ dữ liệu thật phải dựa vào Supabase Auth kết hợp Row Level Security (RLS) cho bảng và Storage.

- Supabase `anon public` key có thể nằm ở frontend **chỉ khi RLS được cấu hình đúng**.
- Tuyệt đối không đưa service role key, token bí mật hoặc khoá riêng tư nào vào HTML/CSS/JavaScript frontend.
- Tài khoản admin dùng chung phải đặt mật khẩu mạnh, không chia sẻ qua kênh không an toàn và nên đổi mật khẩu khi có người không còn phụ trách quản trị.
- Sau khi thao tác xong, đặc biệt trên máy tính dùng chung, hãy bấm **Đăng xuất**.
- Trang admin tự đăng xuất sau 30 phút không hoạt động để giảm rủi ro bỏ quên phiên đăng nhập.
- Chính sách RLS cần giữ nguyên nguyên tắc: khách chỉ đọc sản phẩm đang bật, chỉ được gửi đơn hàng mới, không được đọc/sửa/xoá đơn hàng và không được quản lý products/banners/storage; admin role mới được quản lý products/orders/banners/storage.
...
- **Xoá**: xoá row trong bảng `products` sau khi xác nhận rõ tên/model sản phẩm cần xoá.
